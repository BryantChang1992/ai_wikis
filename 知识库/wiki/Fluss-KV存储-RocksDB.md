...
```
KvManager（全局入口）
  → KvTablet（单 bucket 的 KV 管理单元）
    ├── RocksDBKv              → RocksDB 数据库实例
    ├── WalBuilder             → WAL 构建（三种格式）
    ├── PeriodicSnapshotManager → 定时快照触发
    ├── RowMerger              → 行合并策略（四种实现）
    ├── PartialUpdater         → 部分列更新
    ├── AutoIncrementManager   → 自增 ID
    └── KvPreWriteBuffer       → 预写缓冲
```
...
```
传统方案:   写 → RocksDB WAL + RocksDB SST → 恢复时读 WAL
Fluss 方案: 写 → RocksDB SST + changelog LogTablet → 恢复时重放 LogTablet
```
...
```
PeriodicSnapshotManager（定时触发，周期可配）
  → KvTabletSnapshotTarget（执行快照）
    → RocksIncrementalSnapshot（RocksDB Checkpoint → 仅传输变更 SST）
      → KvSnapshotDataUploader（上传至远程 S3/HDFS）
        → CompletedKvSnapshotCommitter（提交快照元数据）
          → ZooKeeperCompletedSnapshotHandleStore（ZK 记录）
```
