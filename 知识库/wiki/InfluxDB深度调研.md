...
```
InfluxDB 概念体系
├── [[InfluxDB-数据模型]] — Measurement、Tag、Field、Timestamp、Series、Cardinality
├── [[InfluxDB-TSM存储引擎]] — TSM (Time-Structured Merge Tree)、TSI 倒排索引
├── [[InfluxDB-3-列存引擎]] — Parquet/Arrow/DataFusion、存算分离、无索引剪枝
├── [[InfluxDB-写入与查询路径]] — v1/v2 Iterator vs v3 DataFusion 向量化
├── [[InfluxDB-指标设计与基数管理]] — Tag/Field 决策、基数计算、五大最佳实践
├── [[InfluxDB-多副本与高可用]] — Router 双副本、WAL 崩溃恢复、Object Store 3 AZ
└── [[InfluxDB-Catalog元数据]] — PostgreSQL 兼容 RDBMS、BoltDB vs 独立服务
```
