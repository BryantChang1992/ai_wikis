...
Fluss 数据生命周期:
  实时写入 (ms 级)
    → Fluss Tablet（本地 Log + KV，毫秒级读写，OLTP 级别）
      → Tiering 作业（分钟级异步分层）
        → Lakehouse（Parquet/Arrow，分钟级物化，OLAP 级别）
          → 本地 segment 可被 GC（释放磁盘空间）
```
...
```
CoordinatorServer
  → LakeTableTieringManager 调度 tiering 任务
    → TieringSource（Flink Source，从 TabletServer 读本地数据）
      → LakeWriter（写入 Lake Storage：Iceberg/Paimon/Hudi/Lance）
        → TieringCommitter（提交 Snapshot）
          → Coordinator.commitLakeTableSnapshot（通知协调器）
            → TabletServer.notifyLakeTableOffset（更新本地 lake offset）
              → 本地 LogManager 删除已被 tiered 的 segment
```
