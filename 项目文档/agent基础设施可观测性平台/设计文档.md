...
```
P0: 可观测性 + 可靠性     ← 先能看见、能兜底
P1: 工作流自动化          ← 把规范写进 Agent prompt
P2: 记忆管理              ← 分层、可见性
P3: 知识管理              ← 检索、推送、沉淀闭环
```
...
- [ ] **Phase 1 · 可观测性 MVP**（1 周）
  - Log Parser 守护进程 + SQLite 初始化（含 memory_ops / wiki_access 表）
  - `taskboard` CLI
  - Token 采集与统计
- [ ] **Phase 2 · Dashboard 前端**（1 周）
  - 纯静态 HTML + Chart.js，部署到 GitHub Pages
  - 包含：任务看板、Token 汇总、Agent 列表、记忆热力图
- [ ] **Phase 3 · Agent 规范落地**（1-2 周）
  - 将协作规范写入各 Agent 的 AGENTS.md
