...
| 操作 | CEO | VP 层 | PMO | 专家层 | 执行层 |
|------|-----|-------|-----|--------|--------|
| 任命 VP/专家 | ✅ | ❌ | ❌ | ❌ | ❌ |
| 创建子 Agent | ✅ | ✅ 同领域 | ❌ | ✅ sessions_spawn | ❌ |
| 最终决策 | ✅ | 🔶 领域内 | ❌ | 🔶 建议 | ❌ |
| 修改规范 | ✅ 全局 | ✅ 领域内 | ❌ | ❌ | ❌ |
| 写入 Dashboard | ✅ | ✅ | ✅ | ✅ | ✅ |
| 直接操作 Git | ✅ | ✅ | ✅ | ✅ | 🔶 受限 |
| 跨层指令 | ✅ 全局 | 🔶 同领域向下 | ❌ | 🔶 向下 spawn | ❌ |
| 跨层汇报 | — | 🔶 向上汇报 | ✅ 向上 | ✅ 向上 | ✅ 向上 |
| Wiki 发布 | ✅ | 🔶 领域内 | ✅ 全权 | ❌ | ❌ |

> **注**：PMO 汇报给 CTO，通过 `sessions_send` 通信。PMO 不创建子 Agent，但负责所有 Wiki 管理和发布。

### 4.3 通信协议

| 场景 | 方式 | Context | 说明 |
|------|------|---------|------|
| CTO → PMO | `sessions_send` | — | 同级协作，推送进度摘要 |
| CTO → 专家 | `sessions_spawn` | `isolated` | 独立执行，仅收结果 |
| CTO → 执行层 | `sessions_spawn` | `isolated` | 独立执行 |
| 专家 → 执行层 | `sessions_spawn` | `isolated` | 独立执行 |
| 同级 VP 协作 | `sessions_send` | — | 不创建新 session |
| 飞书群内 @ 讨论 | 飞书原生 @ | — | feishu-bot-chat 插件自动转换 |
