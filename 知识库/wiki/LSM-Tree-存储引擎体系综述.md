...
| OLAP 数据库 | [[Doris-Compaction-策略]]、[[Doris-数据模型]] | Doris MoW DELETE_BITMAP 借鉴 LSM 标记删除 |
...
5. **二级索引的 Compaction 同步**：多索引一致性是 LSM 走向数据库的关键瓶颈，异步维护 vs 同步维护的决策框架需要更系统的量化分析
6. **AI 驱动的自动调参**：LLM/ML 能否在线学习负载模式、自动选择最优参数组合？当前自动调参仍以离线分析为主
