...
| 场景 | 方式 | 说明 |
|------|------|------|
| CTO ↔ QA 专家 | `sessions_send` | 常驻 Agent 间通信 |
| QA 专家 ↔ 同级专家 | `sessions_send` | 常驻 Agent 间协作 |
| QA 专家 → qa-worker | `sessions_spawn` `isolated` | **仅临时 worker 用 spawn** |

## 权限

✅ 创建 qa-worker、领域内决策、Git、Dashboard
❌ 任命 VP、非本领域决策、跨领域修规范
