...
```
RpcGateway (基础: hostname + port)
  ├── TabletServerGateway  (produceLog/fetchLog/putKv/lookup/limitScan/prefixLookup...)
  ├── CoordinatorGateway   (createDatabase/createTable/alterTable/metadata/adjustIsr/rebalance...)
  └── AdminGateway         (createDatabase/listDatabases/createTable/listTables/getTableSchema...)
```
...
invoke(proxy, method, args):
  1. ApiManager.forMethodName(method.getName()) → 查找 ApiKeys 匹配
  2. apiMethod.serializeRequest(method, args)    → Protobuf 序列化
  3. nettyClient.send(serverAddress, request)     → Netty 异步发送
  4. apiMethod.deserializeResponse(response, method) → 反序列化
```
...
```
Netty Pipeline: Decoder → RequestHandlerSelector
                            ├── FlussRequestHandler (Protobuf)
                            └── KafkaRequestHandler (Kafka 二进制协议)
```
