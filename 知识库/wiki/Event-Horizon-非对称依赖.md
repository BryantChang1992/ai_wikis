...
| 强一致 | [[Spanner]] / Paxos / [[2PC]] 全序 | **延迟根本意义上无法突破**（跨地域 replica 通信） |
...
|------|---------|-----------|---------------|
| [[Spanner]] / 严格可串行化 | 全序 | N/A（无弱操作概念） | 所有操作 |
...
- **[[Calvin]]**：Calvin 通过确定性执行避免 2PC，但仍在每个 epoch 对事务全排序。SL 可以进一步降低 epoch 内"非关键事务"的排序代价。
