...
> 非强制，指导性。标签粒度建议：领域标签 + 核心技术名 + 关注点（如 `存储引擎` `LSM-Tree` `写优化`）。
...
3. Wiki 卡片增量 ≥ 10 张时，触发全量 Lint（不限于周五）
4. 所有 dangling 引用必须在同一次会话中修完才能 commit
...
- [ ] 内容非纯摘要——包含论文章节号、系统对比数据、架构图等深度信息

### 正文 [[wikilink]] 使用规范

- 每个概念**仅在首次出现时**用 `[[wikilink]]` 包裹一次
- 同一段落内不要重复链同一个概念
- 正确示例：`LSM-Tree 的 Compaction 策略影响写放大、读性能和空间效率（详见 [[LSM-Tree]]）。`
- 错误示例：`LSM-Tree 的 Compaction 策略影响 [[LSM-Tree]] 的写放大，[[LSM-Tree]] 的读性能，以及 [[LSM-Tree]] 的空间效率。`
