...
```
 Epoch Allocation Ladder
 ═══════════════════════════════════════════════════
 
                                   去中心化程度 ↑
  Pre-allocation  ──────────────  最保守（Classic Paxos）
       │
  Epochs by Allocator (§7.1)  ─  2 RTT，单点瓶颈
       │
  Epochs by Value Mapping (§7.2)  无协调器，但 epoch 由 value 决定
       │
  Epochs by Recovery (§7.3)  ───  允许多 proposer 同 epoch，去中心化突破
       │
  Multi-path Paxos (§7.4)  ─────  混合方案，自适应回退
```
...
Proposer                    Allocator                Acceptors
   ...
   │◀─── Accept(e_min, v) ──────┼────────────────────────│
   │                            │                        │
  2 RTT total: 1 RTT (allocator) + 1 RTT (acceptors)
```
...
```
Multi-path Paxos 执行路径：

   Proposer 启动
        │
        ▼
   ┌─ Allocator 可用？───Yes──▶ 获取 e_min ──▶ 跳过 Phase-1 ──▶ 2 RTT 完成
   │                                                  (路径 A:最快)
   │
   No
   │
   ▼
   ┌─ 知道 candidate value？───Yes──▶ hash(v) → epoch ──▶ Phase-1 + Phase-2
   │                                                  (路径 B:Epochs by Value)
   No
   │
   ▼
   ┌─ 选择任意 epoch ──▶ Phase-1 + Phase-2 ──▶ 冲突？──▶ Recovery
   │                                        (路径 C:Epochs by Recovery)
   │
   ▼
   Classic Paxos fallback (路径 D:兜底)
```
...
Epochs by Recovery **不需要** fast quorum 和 classic quorum 之间的额外 intersection 要求。原因：

```
Fast Paxos 的问题：
  ┌──── fast quorum ────┐     ┌── classic quorum ──┐
  │  v₁ (from P₁)      │     │  v₂ (from P₂)      │
  │                    │     │                    │
  │ 可能不相交！       │     │                    │
  └────────────────────┘     └────────────────────┘
  → 如果 Q_fast ∩ Q_classic = ∅，v₁ 和 v₂ 可能同时被 commit
  → 因此必须强制 f + c > n

Epochs by Recovery 的方案：
  ┌──── epoch e ────────┐
  │  P₁ proposes v      │     Property 17: 同一 epoch 同一 value
  │  P₂ proposes v      │     → 不同 value 的冲突不会发生在同 epoch
  │  P₃ proposes v      │     → 无需额外的 quorum 约束
  └─────────────────────┘
```
